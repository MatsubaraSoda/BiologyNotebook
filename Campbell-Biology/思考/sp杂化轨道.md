...
基态的碳原子只有 2 个未成对电子。如果不做出任何改变，它理应只能和 2 个氢原子结合，形成 $\mathrm{CH_2}$ 。这无法解释为什么碳能结合 4 个氢原子形成 $\mathrm{CH_4}$ 。

### 二、事实与矛盾

事实二：考虑甲烷 $\mathrm{CH_4}$ 的空间结构，甲烷是完美的正四面体，四个 $\mathrm{C{-}H}$ 键完全等价，键角均为 109.5°。
...
即使假设键长、键能的差异小到忽略不计，也无法解释空间构型。$p_x, p_y, p_z$ 这三个轨道在三维空间中是互相垂直的（沿 X、Y、Z 轴分布）。这意味着，有这三个 $p$ 轨道形成的三个 $\mathrm{C{-}H}$ 键，它们之间的夹角必须是 90°。这与正四面体的性质相去甚远。
